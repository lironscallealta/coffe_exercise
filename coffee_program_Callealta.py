...
list_money_amounts_strings = [string_quarters, string_dimes, string_nickels, string_pennies]

messages = [order_done_msj, order_options, turn_off, h_short,
            string_quarters, string_dimes, string_pennies,
...
def checking_integrates(number_or_not, amount_in_string):
    ...
    while not number_or_not.isdigit():
        number_or_not = input(f"Please write it well.\n{h_short} {amount_in_string}: ?: ")
...
def money_user_dict() -> list: #with dict
    ...
    user_cash = []
    while True:
        money_user = input(f"{h_short} {list_money_amounts_strings[counting]}?: ")  # h = how many?
        dict_info_money = checking_integrates(money_user, list_money_amounts_strings[counting])
